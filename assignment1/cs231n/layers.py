...
def svm_loss(x, y):
    ...
    ###########################################################################
    N, C = x.shape
    correct_scores = x[np.arange(N), y].reshape(-1, 1)
    margins = np.maximum(0, x - correct_scores + 1)  # note the 1 for the delta
